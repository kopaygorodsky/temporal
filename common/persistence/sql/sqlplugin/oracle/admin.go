...
func (mdb *db) Exec(stmt string, args ...interface{}) error {
	...
	_, err = db.Exec(mdb.sanitizeStmt(stmt), args...)
	return mdb.handle.ConvertError(err)
}

const sqlEndKeyword = "END"

//make it handle large queries not formatted
func (mdb *db) sanitizeStmt(stmt string) string {
	if len(stmt) < len(sqlEndKeyword)+1 {
		return stmt
	}

	// Check if string ends with semicolon
	if strings.HasSuffix(stmt, ";") {
		if !strings.HasSuffix(strings.ToUpper(stmt), "END;") {
...
func (mdb *db) NamedExec(stmt string, args map[string]interface{}) error {
	...
	_, err = db.NamedExec(mdb.sanitizeStmt(stmt), args)
	return mdb.handle.ConvertError(err)
}
...
func (mdb *db) DropDatabase(name string) error {
	// a placeholder as this is typically a DBA operation in Oracle
